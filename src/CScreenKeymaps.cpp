...
void CScreenKeymaps::HandleKeyDown(BYTE nKey)
{
    ...
    if (nKey == VK_DELETE || nKey == VK_BACK) {
        ...
        nKey = g_pBaldurChitin->m_pObjectGame->sub_5A97D0(g_pBaldurChitin->m_pObjectGame->sub_5A9780(nKey));
        if (nKey < 0) {
            return;
        }
    }
    ...
    CString sValue;
    if (m_bSystemKeyCtrl) {
        sValue.Format("%s%c", FetchString(24638), g_pBaldurChitin->m_pObjectGame->sub_5A9780(nKey));
    } else {
        sValue.Format("%c", g_pBaldurChitin->m_pObjectGame->sub_5A9780(nKey));
    }
...
void CScreenKeymaps::UpdateMainPanel()
{
    ...
    for (int index = 0; index < CSCREENKEYMAPS_PER_PAGE; index++) {
        ...
        CUIControlLabel* pKeyLabel = static_cast<CUIControlLabel*>(pPanel->GetControl(index + 0x10000005));
        if (dword_8B3FE0[m_nPage][index] >= 0) {
            int ch = g_pBaldurChitin->m_pObjectGame->sub_5A9780(m_pKeymap[dword_8B3FE0[m_nPage][index]]);
            if (ch > 0) {
...
void CScreenKeymaps::StartKeymaps()
{
    ...
    for (SHORT index = 0; index < CINFGAME_KEYMAP_SIZE; index++) {
        ...
        m_pKeymap[index] = g_pBaldurChitin->m_pObjectGame->m_pKeymap[index];
        m_pKeymapFlags[index] = g_pBaldurChitin->m_pObjectGame->m_pKeymapFlags[index];
    }
}
...
{
    if (m_nSelectedKeymapIndex >= 0) {
        BYTE nKey = g_pBaldurChitin->m_pObjectGame->sub_5A9830(CScreenKeymaps::dword_8B3FE0[m_nPage][m_nSelectedKeymapIndex]);
        if (nKey != 0) {
            HandleKeyDown(nKey);
        } else {
            HandleKeyDown(0);
...
void CScreenKeymaps::OnDoneButtonClick()
{
    ...
    for (int index = 0; index < CINFGAME_KEYMAP_SIZE; index++) {
        g_pBaldurChitin->m_pObjectGame->SetKey(index,
            m_pKeymap[index],
...
void CScreenKeymaps::OnErrorButtonClick(INT nButton)
{
    ...
    case 33484:
        ...
        case 0:
            ...
            for (int index = 0; index < CINFGAME_KEYMAP_SIZE; index++) {
                m_pKeymap[index] = g_pBaldurChitin->m_pObjectGame->sub_5A9830(index);
            }
...
void CScreenKeymaps::SetKeymap(INT nPage, INT nIndex, BYTE nKey, BYTE nKeyFlag)
{
    ...
    if (nKeyFlag != 0) {
        sKey.Format("%s%c",
            FetchString(24638),
            g_pBaldurChitin->m_pObjectGame->sub_5A9780(m_pKeymap[dword_8B3FE0[nPage][nIndex]]));
    } else {
        sKey.Format("%c", g_pBaldurChitin->m_pObjectGame->sub_5A9780(m_pKeymap[dword_8B3FE0[nPage][nIndex]]));
    }
